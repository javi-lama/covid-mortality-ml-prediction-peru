...
# ==============================================================================
# SECTION 2B: LOAD GOLD-STANDARD LOGISTIC REGRESSION
# ==============================================================================

cat("\n--- SECTION 2B: Loading Gold-Standard Logistic Regression ---\n")

# Check if gold-standard LogReg exists (created by Logistic_Regression_GoldStandard.R)
gold_logreg_exists <- file.exists("preds_logreg_goldstandard.rds") &&
                      file.exists("roc_logreg_goldstandard.rds")

if (gold_logreg_exists) {
  # Load gold-standard predictions and ROC object
  preds_logreg_gold <- readRDS("preds_logreg_goldstandard.rds")
  roc_logreg_gold <- readRDS("roc_logreg_goldstandard.rds")
  model_logreg_gold <- readRDS("model_logreg_goldstandard.rds")

  cat("Gold-Standard LogReg loaded successfully\n")
  cat("  - Lasso regularization: YES\n")
  cat("  - CV-derived threshold: YES\n")
  cat("  - Feature engineering equity: YES\n")
} else {
  cat("Gold-Standard LogReg not found.\n")
  cat("Run r/Logistic_Regression_GoldStandard.R first to generate artifacts.\n")
  preds_logreg_gold <- NULL
  roc_logreg_gold <- NULL
}
...
# Add Gold-Standard LogReg if available
if (gold_logreg_exists && !is.null(roc_logreg_gold)) {
  roc_list[["LogReg (Gold Standard)"]] <- roc_logreg_gold
  preds_list[["LogReg (Gold Standard)"]] <- preds_logreg_gold
  cat("\nGold-Standard LogReg added to comparison lists\n")
}
...
# Note if Gold-Standard LogReg is included
if (gold_logreg_exists) {
  cat("\nNOTE: Gold-Standard LogReg (Lasso) included in roc_list and preds_list\n")
  cat("Total models in comparison:", length(roc_list), "\n")
}
